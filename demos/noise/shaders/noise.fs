...
void main()
{
  ...
  second.x = perlin_noise(noise_texture, offset + 2 * first + vec2(0.1111, 0.1222) * vec2(-0.14,-0.09) * time * 1.31, 9);
  second.y = perlin_noise(noise_texture, offset + 2 * first + vec2(0.1652, 0.2111) * vec2(0.16,-0.14) * time * 1.44, 10);
  ...
  color = mix(color, gradient_lookup(g_two, first.x * 0.5), second.y * 2);
  color = mix(color, gradient_lookup(g_second, first.y * 1), second.x * second.y);
