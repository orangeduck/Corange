...
void main()
{
	...
	vec2 bump_normal1 = texture2D(tex_noise1, position.xz / 15.0 + vec2(0.05, 0.05) * time * 1.2).rg;
	vec2 bump_normal2 = texture2D(tex_noise2, position.xz / 11.0 + vec2(-0.05,-0.05) * time * 1.41).rg;
	vec2 bump_normal3 = texture2D(tex_noise3, position.xz / 08.0 + vec2(0.051, 0.034) * time * 0.86).rg;
	vec2 bump_normal4 = texture2D(tex_noise4, position.xz / 24.0 + vec2(0.062, -0.012) * time * 0.99).rg;
	vec2 bump_normal5 = texture2D(tex_noise5, position.xz / 33.0 + vec2(-0.061, 0.081) * time * 1.1).rg;
