...
void depth_mapper_init(camera* c) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, viewport_width(), viewport_height(), 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
...
static void depth_mapper_setup_camera() {
  
  CAMERA->target = v3_neg(CAMERA->target);
