...
void main()
{
	...
	if( normal_to_screen >= 0.0 ) {
		...
		vec4 screen_position = proj_matrix * view_matrix * world_position;
		
		screen_uvs = (screen_position.xy / screen_position.w) / 2 + 0.5;
		...
		float hash_value = mod( (world_position.x + world_position.y + world_position.z) * 1216 , 3.0 );
