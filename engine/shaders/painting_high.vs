...
void main()
{
	...
	if( normal_to_screen > 0.0 ) {
		...
		float hash_value = mod( world_position.x + world_position.y + world_position.z , 1.0 );
		
		if ( hash_value < (total_size * total_size) ) {
			...
			float scale = 1.75 * (skip + 2) * max(-normal_to_screen, 0.0);
